...
def test_camelcase_pr_audit_aliases(monkeypatch):
    ...
    PrAuditJiraBoards → JIRA_BOARDS (same key used by standup — DRY).
    """
    monkeypatch.delenv("PR_AUDIT_TEAM_MEMBERS", raising=False)
    monkeypatch.delenv("JIRA_BOARDS", raising=False)
    ...
    assert config["JIRA_BOARDS"] == "123,456"       # PrAuditJiraBoards → JIRA_BOARDS
    assert config["PR_AUDIT_GH_MILESTONE"] == "v2.5"


def test_team_id_alias_maps_to_jira_space(monkeypatch):
    """TeamId org-config alias resolves to JIRA_SPACE — no separate TEAM_ID key."""
    monkeypatch.delenv("JIRA_SPACE", raising=False)
    monkeypatch.setenv("TeamId", "PlatformCore")
    config = load_config()
    assert config["JIRA_SPACE"] == "PlatformCore"


def test_pr_audit_defaults_are_empty_strings():
    """PR audit keys have empty-string defaults (non-fatal when unset)."""
    config = load_config()
    assert config.get("PR_AUDIT_TEAM_MEMBERS", "MISSING") != "MISSING"
    assert config.get("JIRA_BOARDS", "MISSING") != "MISSING"        # shared with standup
