...
def test_set_config_dry_run_prints_without_pushing(capsys):
    ...
    with patch("scripts.slack.deploy_workflows._resolve_org_id", return_value="org-abc"), \
         patch("scripts.slack.deploy_workflows._push_org_config") as mock_push, \
         patch("os.environ.get", side_effect=lambda k, d="": "fake-token" if k == "FORMICARY_TOKEN" else d):
