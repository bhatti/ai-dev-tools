...
SYSTEM_PROMPTS = {
    ...
    "pr_audit": (
        ...
        "(4) A PR that shows 'Approved by: <name>' was reviewed by a human — do NOT flag it as 'no human review' even if the approver left no inline comments. Only flag PRs with no approvers AND no human comments. "
        "(5) When 'has_acceptance_criteria: false', read the Issue description excerpt. If it clearly describes desired behavior or a specific fix approach, that counts as requirements. Only flag AC as missing if the description has no testable expected behavior at all. "
        "(6) Recommendations must NOT cite specific LOC thresholds or repo directory paths as policy gates — use risk categories (auth, flags, infra) instead. "
        "(7) Rubber-stamp detection: use the pre-computed 'Rubber-stamp approvers' and 'Substantive human comments' fields. "
        "Rubber-stamp = approver left zero substantive comments (silent approval, LGTM, +1, looks good, emoji-only). "
        "ONLY flag rubber-stamp as a finding on HIGH blast-radius changes (auth/billing/query-engine/infra/config/feature-flags). "
        "Rubber-stamp on low-risk, self-contained changes is ACCEPTABLE and should NOT be flagged. "
        "Severity: rubber-stamp on HIGH blast-radius = HIGH; on MEDIUM blast-radius = MEDIUM; on LOW blast-radius = skip. "
        "(8) Bot-authored PRs: use the pre-computed 'Author type: AI/bot-authored PR' field. "
        "Bot-authored PRs require at minimum 1 substantive human comment from a reviewer who can verify correctness — "
        "not just a silent approval or LGTM. Flag bot-authored PRs with 0 substantive comments as MEDIUM+ risk. "
        "Distinguish: 'no review on bot PR' (MEDIUM — higher bar needed for bots) vs 'no review on human PR' (informational if low blast-radius). "
