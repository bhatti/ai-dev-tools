...
def _resolve_repo_url(config: dict, explicit_url: str | None) -> str | None:
...
def _repo_label(config: dict, repo_url: str | None) -> str:
...
def _clone_for_audit(repo_url: str, branch: str, dest: Path, depth: int = 500) -> tuple[bool, str]:
...
def main(repo_url: str | None, branch: str | None, commits: int | None, focus: str | None, skill: str) -> None:
    ...
    resolved_url = _resolve_repo_url(config, repo_url)
    repo_label = _repo_label(config, resolved_url)
    ...
    if resolved_url:
        ...
        success, branch = _clone_for_audit(resolved_url, branch, repo_path, depth=clone_depth)
        if not success:
        ...
        try:
            ...
            if remote_url:
                repo_label = _repo_label(config, remote_url)
        except Exception:
            pass
