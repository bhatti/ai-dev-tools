"""Launch a K8s pipeline Job for a picked Jira issue.

Usage:
    python -m scripts.jira.launch_pipeline --issue-id PROJ-42

Called by jira/issue_picker.py after successfully writing issue.json.
...
Optional env:
    PIPELINE_JOB_TEMPLATE  — path to the pipeline Job YAML template
                              (default: /app/k8s/jira-pipeline-job.yaml)
...
def main(issue_id: str) -> None:
    ...
    template = os.environ.get("PIPELINE_JOB_TEMPLATE", "/app/k8s/jira-pipeline-job.yaml")
